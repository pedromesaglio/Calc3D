#!/usr/bin/env python3
"""
Script para crear usuario 'Amigo de la casa' en PRODUCCIÓN (Railway)
Detecta automáticamente si está en Railway y usa la DB correcta
"""
import sqlite3
import os
...
def get_db_path():
    """Detecta la ruta correcta de la base de datos"""
    if os.getenv("RAILWAY_ENVIRONMENT"):
        db_dir = Path("/data")
        db_dir.mkdir(parents=True, exist_ok=True)
        return db_dir / "calc3d.db"
    else:
        return Path("calc3d.db")
...
def create_friend_user():
    ...
    if not usage:
        ...
        print(f"✓ Límites de uso reseteados")

    conn.commit()

    # Mostrar información final
    print(f"\n📋 Usuario configurado en {'PRODUCCIÓN (Railway)' if os.getenv('RAILWAY_ENVIRONMENT') else 'LOCAL'}:")
